...
public class Self extends AppCompatActivity implements View.OnClickListener, listener {
    ...
    @Override
    protected void onCreate(Bundle SelfInstanceState){
        super.onCreate(SelfInstanceState);
    ...
    public void OnItemClick(int position, int source) {
        if(source == 3){
            ...
            Intent intent = new Intent(Self.this, ProfileStalking.class);
        ...
        if(source == 1){
            ...
            Intent intent = new Intent(Self.this, Comment.class);
            startActivity(intent);
        }
        if (source == 2) {
            ...
            Comment.postId = Id;
            adapter.notifyItemChanged(position);
            DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("posts").child(Id).child("Self");
            databaseReference.child(MainActivity.userId).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if (dataSnapshot.exists()) {
                        DatabaseReference databaseReference1 = FirebaseDatabase.getInstance().getReference("posts").child(Comment.postId).child("Self");
        ...
        if(source == 4){
            ...
            Intent intent = new Intent(Self.this, Image.class);
            startActivity(intent);
        }
    }
}
